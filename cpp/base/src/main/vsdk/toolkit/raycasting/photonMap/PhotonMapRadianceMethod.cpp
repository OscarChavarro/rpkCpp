...
#include "vsdk/toolkit/raycasting/bidirectionalRaytracing/LightDirSampler.h"
#include "vsdk/toolkit/raycasting/photonMap/PhotonMapSampler.h"
#include "vsdk/toolkit/raycasting/photonMap/ScreenSampler.h"
#include "vsdk/toolkit/raycasting/photonMap/Photon.h"
#include "vsdk/toolkit/raycasting/photonMap/PhotonMapRadianceMethod.h"
    ...
    const RadianceMethod *radianceMethod)
{
    ...
    if ( photonMapState.doGlobalMap ) {
        ...
        photonMapConfig.map->printStats(&java::System::err);
    }
...
PhotonMapRadianceMethod::getStats() const {
    ...
    if ( photonMapConfig.map ) {
        ...
        if ( statsOffset < STRING_LENGTH - 1 ) {
            photonMapConfig.map->getStats(&stats[statsOffset], STRING_LENGTH - statsOffset);
