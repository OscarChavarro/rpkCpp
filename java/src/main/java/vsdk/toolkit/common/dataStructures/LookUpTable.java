...
public class LookUpTable<T> {
    private LookUpBehaviors behaviorType;
    private int currentTableSize;
    private LookUpEntity<T>[] table;
    ...
    public LookUpTable() {
        this(LookUpBehaviors.NON_OWNING);
    }

    public LookUpTable(LookUpBehaviors behaviorType) {
    ...
    */
    private static long lookUpShuffleHash(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            int b = text.charAt(i) & 0xff;
            hash ^= (long)SHUFFLE[b] << ((bitShift += 11) & 0xf);
        }
    ...
    */
    ...
    public int lookUpInit(int nel) {
        ...
        table = (LookUpEntity<T>[])new LookUpEntity[currentTableSize];
        if (table == null) {
            currentTableSize = 0;
            return 0;
        }
    ...
    */
    public LookUpEntity<T> lookUpFind(String key) {
        ...
        long hashValue = lookUpShuffleHash(key);

        while (true) {
            int index = (int)(hashValue % currentTableSize);
            if (index < 0) {
            ...
            if (lookUpReAlloc(currentTableSize - numberOfDeletedEntries + 1) == 0) {
                return null;
            }
    ...
    */
    private int lookUpReAlloc(int nel) {
        LookUpEntity<T>[] oldTable = table;
        int oldTableSize = currentTableSize;
        int oldDeletedEntries = numberOfDeletedEntries;
        ...
        for (int i = 0; i < oldTableSize; i++) {
            LookUpEntity<T> entry = oldTable[i];
            if (entry.key == null) {
                continue;
            }
            if (entry.data != null) {
