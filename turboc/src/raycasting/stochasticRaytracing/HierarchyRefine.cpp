    ...
    const RenderOptions *renderOptions)
{
    ...
    if ( rcv->isCluster() ) {
        ...
        if ( !rcv->regularSubElements ) {
            StochasticRadiosityElement::stchsRadElemRegSbdvdElem(rcv, renderOptions);
        }
        rcv = StochasticRadiosityElement::stchsRadElemRegSubElemAPnt(rcv, ur, vr);
    }
    link->rcv = rcv;
    return link;
}
    ...
    const RenderOptions *renderOptions)
{
    ...
    if ( src->isCluster() ) {
        ...
        if ( !src->regularSubElements ) {
            StochasticRadiosityElement::stchsRadElemRegSbdvdElem(src, renderOptions);
        }
        src = StochasticRadiosityElement::stchsRadElemRegSubElemAPnt(src, us, vs);
    }
    link->src = src;
    return link;
}
...
Hierarchy::formFactorEstimate(const StochasticRadiosityElement *rcv, const StochasticRadiosityElement *src) {
    ...
    double f2 = 2.0 * f;
    double c1 = rcv->isCluster() ? 1.0 /*0.25*/ : Math::abs(D.dotProduct(rcv->patch->normal)) / d;
    if ( c1 < f2 ) {
        c1 = f2;
    }
    double c2 = src->isCluster() ? 1.0 /*0.25*/ : Math::abs(D.dotProduct(src->patch->normal)) / d;
    if ( c2 < f2 ) {
        c2 = f2;
    }
    ...
    const Statistics *statistics)
{
    ...
    if ( StochasticRelaxation::activeState().importanceDriven ) {
        ...
        if ( !rcv->isCluster() ) {
            propagatedPower *= StochasticRadiosityElement::stchsRadElemSclrRefl(rcv);
        }
    }
...
Hierarchy::subDivideLargest(const Link *link) {
    ...
    if ( rcv->area < ElementHierarchyState::activeState().minimumArea && src->area < ElementHierarchyState::activeState().minimumArea ) {
        return ((REFINE_ACTION)(Hierarchy::dontRefineCallBack));
    } else {
...
Hierarchy::powerOracle(const Link *link) {
    if ( selfLink(link) ) {
        return ((REFINE_ACTION)(Hierarchy::subdivideReceiverCallBack));
    ...
    const RenderOptions *renderOptions)
{
    if ( !ElementHierarchyState::activeState().do_h_meshing ) {
        ...
        while ( (action = evaluateLink(link)) != ((REFINE_ACTION)(Hierarchy::dontRefineCallBack)) ) {
