...
Tracepath::PatchProbabilityCallback Tracepath::birthProbability = NULL;
double Tracepath::sumProbabilities = 0.0;

/**
Initialises numberOfNodes, nodes allocated to zero and 'nodes' to the NULL pointer
*/
void
    ...
    Path *path)
{
    ...
    do {
        ...
        ray = Localline::mcrGenerateLocalLine(P, Sample4d::sample4D(((unsigned int)(McradP::topLvlStochRadElem(P)->rayIndex))));
        McradP::topLvlStochRadElem(P)->rayIndex++;
    ...
    const ArrayList<Patch *> *scenePatches)
{
    ...
    for ( int i = 0; scenePatches != NULL && i < scenePatches->size(); i++ ) {
        ...
        long paths_this_patch = ((int)(Math::floor((pCumulative + p) * ((double)(numberOfPaths)) + rnd))) - pathCount;
