...
void
BidirectionalPathRaytracer::initialize(const ArrayList<Patch *> *lightPatches) const {
    ...
    const RenderOptions * /*renderOptions*/) const
{
    ...
    config.baseConfig = new BidirPathRaytrcCnfg;
...
BidirectionalPathRaytracer::handlePathX0(
    ...
    BiPath *path)
{
    const PhongEmitDistFunc *endingEdf = path->m_eyeEndNode->m_hit.getMaterial() ? path->m_eyeEndNode->m_hit.getMaterial()->getEdf() : NULL;
    const bool endingInEnvironment = path->m_eyeEndNode->m_rayType == ENVIRONMENT;
    const bool hasEnvironmentBackground = endingInEnvironment && sceneBackground != NULL;
    ...
    if ( endingEdf != NULL || hasEnvironmentBackground || config->baseConfig->useSpars ) {
...
BidirectionalPathRaytracer::bpCalcPixel(
    ...
    void *data)
{
    BidirPathTraceConfig *config = ((BidirPathTraceConfig *)(data));
    double x1;
    double x2;
    ColorRgb result;
    ...
    for ( int i = 0; i < config->baseConfig->samplesPerPixel; i++ ) {
        if ( config->eyeConfig.maxDepth > 1 ) {
            ...
            if ( config->eyeConfig.dirSampler->sample(camera, sceneVoxelGrid, sceneBackground, NULL, config->eyePath, pixNode, x1, x2) ) {
...
BidirectionalPathRaytracer::doBptAndSubsequentImages(
    ...
    VoxelGrid *sceneVoxelGrid,
    Background *sceneBackground,
    BidirPathTraceConfig *config) const
{
    int maxSamples;
    int nrIterations;
    ...
    System::out.printf("nrIter %i, maxSamples %i, origSamples %i\n", nrIterations,
    ...
    for ( int i = 0; i < nrIterations; i++ ) {
        ...
        // Save images
        if ( format1[0] != '\0' ) {
            Formatter::format(filename, STRINGS_SIZE, format1, totalSamples);
            config->screen->writeFile(filename);
        }
        if ( format2[0] != '\0' ) {
            Formatter::format(filename, STRINGS_SIZE, format2, totalSamples);
            config->screen->writeFile(filename);
        }
    ...
    BidirPathTraceConfig *config) const
{
    ...
    System::out.printf("Doing %i iterations, thus %i samples per pixel\n", numberOfIterations, maxSamples);
    ...
    for ( int i = 1; i <= numberOfIterations; i++ ) {
        System::out.printf("Doing run with %i samples, %i samples already done\n", newSPP, oldTotalSPP);
        ...
        config->dest->render();
        Formatter::format(fileName, STRINGS_SIZE, "deScreen%i.ppm.gz", newTotalSPP);

        if ( config->dest2 ) {
            config->dest2->render();
            Formatter::format(fileName, STRINGS_SIZE, "de2Screen%i.ppm.gz", newTotalSPP);

            // Merge two images (just add!) into screen
            config->screen->merge(config->dest, config->dest2, camera);
            config->screen->render();
            Formatter::format(fileName, STRINGS_SIZE, "deMRGScreen%i.ppm.gz", newTotalSPP);
        } else {
