...
class LightList: private CircularList<LightInfo>{ private:
  ...
  public:
    ...
    explicit LightList(const ArrayList<Patch *> *list, bool includeVirtualPatches = false);
  ...
  private:
    ...
    static double
    computeOneLightImportance( const Patch *light, const Vector3D *point, const Vector3D *normal, float emittedFlux);

    // Specialisations by patch type (normal or virtual) of ComputeOneLightImportance
    static double
    cmptOneLightImpVrtl( const Patch *light, const Vector3D *, const Vector3D *, float);

    static double
    computeOneLightImportanceReal( const Patch *light, const Vector3D *point, const Vector3D *normal, float emittedFlux);
