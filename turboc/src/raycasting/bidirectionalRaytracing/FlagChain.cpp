...
FlagChain::FlagChain(const int paramLength, const bool paramSubtract): length(), subtract(){ chain = NULL;

    init(paramLength, paramSubtract);
}

FlagChain::FlagChain(const FlagChain &c): chain(), length(), subtract(){ init(c.length, c.subtract);

    if ( c.chain != NULL && chain != NULL){ for ( int i = 0; i < length; i++){ chain[i] = c.chain[i];
        }
    }
}

FlagChain::~FlagChain(){ delete[] chain;
}

void
FlagChain::init(const int inLength, bool const inSubtract){ if ( chain != NULL){ delete[] chain;
    }

    length = inLength;
    subtract = inSubtract;
    if ( inLength > 0){ chain = new char[inLength];
        for ( int i = 0; i < inLength; i++){ chain[i] = 0;
        }
    } else{ chain = NULL;
    }
}
...
bool
FlagChain::compare(const FlagChain *c1, const FlagChain *c2){ // Determine if equal

    int nrDifferent = 0;

    if ( (c1->length != c2->length) || (c1->subtract != c2->subtract)){ return false;
    }

    for ( int i = 0; (i < c1->length) && (nrDifferent == 0); i++){ if ( c1->chain[i] != c2->chain[i]){ nrDifferent++;
        }
    }

    // combine into new chain

    if ( nrDifferent == 0){ // flag chains identical
        return true;
    }
...
FlagChain *
FlagChain::combine(const FlagChain *c1, const FlagChain *c2){ // Determine if combinable
    int nrDifferent = 0;
    int diffIndex = 0;

    if ( (c1->length != c2->length) || (c1->subtract != c2->subtract)){ return NULL;
    }

    for ( int i = 0; (i < c1->length) && (nrDifferent <= 1); i++){ if ( c1->chain[i] != c2->chain[i]){ nrDifferent++;
            diffIndex = i;
        }
    }

    // Combine into new chain
    if ( nrDifferent == 0){ // Flag chains identical - maybe dangerous if someone wants to
        // count one contribution twice...
        return new FlagChain(*c1);
    }

    if ( nrDifferent == 1){ // Combinable
        FlagChain *newFlagChain = new FlagChain(*c1);
        newFlagChain->chain[diffIndex] = ((char)(c1->chain[diffIndex] | c2->chain[diffIndex]));
        return newFlagChain;
    }
...
FlagChainList::FlagChainList(){ count = 0;
    length = 0;
}

FlagChainList::~FlagChainList(){ removeAll();
}

void
FlagChainList::add(FlagChainList *list){ // Add all chains in 'list'
    FlagChainIterator iter(*list);
    const FlagChain *tmpChain;

    while ( (tmpChain = iter.nextOnSequence()) != NULL){ add(*tmpChain);
    }
}
...
FlagChainList::addDisjoint(const FlagChain &chain){ if ( count > 0){ if ( chain.length != length){ Error::error("CChainList::add", "Wrong length flag chain inserted!");
    ...
    while ((tmpChain = iter.nextOnSequence()) && !found){ found = FlagChain::compare(tmpChain, &chain);
    }
...
*/
FlagChainList *
FlagChainList::simplify(){ // Try a simple simplification scheme, just comparing pair wise chains
    ...
    if ( c1){ while ( (c2 = iter.nextOnSequence()) != NULL){ cCombined = FlagChain::combine(c1, c2);
            if ( cCombined){ c1 = cCombined; // Combined
            } else{ newList->add(*c1);
                c1 = c2;
            }
        }

        // Add  chain still in c1
        newList->add(*c1);
    }
...
ContribHandler::ContribHandler(){ array = NULL;
    maxLength = 0;
}

void
ContribHandler::init(int paramMaxLength){ this->maxLength = paramMaxLength;
...
ContribHandler::~ContribHandler(){ delete[] array;
}
...
void
ContribHandler::doRegExp(char *regExp, bool subtract){ doRegExpGeneral(regExp, subtract);
}
...
*/
void
ContribHandler::addRegExp(char *regExp){ if ( regExp[0] == '-'){ doRegExp(&regExp[1], true);
    } else{ doRegExp(regExp, false);
    }
}
...
bool
ContribHandler::getFlags(const char *regExp, int *pos, char *flags){ char c;
    int p = *pos;

    *flags = 0;

    if ( regExp[p++] != '('){ doSyntaxError("getFlags: '(' expected");
        return false;
    }

    while ( (c = regExp[p++]) != ')'){ switch ( c){ case 'S':
                switch ( regExp[p]){ case 'T':
                        p++;
            ...
            case 'X':
                ...
                break;
            case 'L':
                if ( regExp[p] != 'X'){ doSyntaxError("getFlags: No 'X' after 'L'. Only LX supported");
                    return false;
                }
                p++;
                *flags = BsdfComponentInfo::BSDF_ALL_COMPONENTS;
                break;
            case 'E':
                if ( regExp[p] != 'X'){ doSyntaxError("getFlags: No 'X' after 'E'. Only EX supported");
                    return false;
                }
                p++;
                *flags = BsdfComponentInfo::BSDF_ALL_COMPONENTS;
...
bool
ContribHandler::getToken(const char *regExp, int *pos, char *token, char *flags){ switch ( regExp[*pos]){ case '\0':
            return false;
        case '+':
            *token = '+';
            (*pos)++;
...
void
ContribHandler::doRegExpGeneral(const char *regExp, bool subtract){ FlagChain c;
    ...
    while ( getToken(regExp, &pos, &token, &data)){ if ( token == 'F'){ // A flag was read

            if ( tokenCount == MAX_REGEXP_ITEMS - 1){ doSyntaxError("Too many tokens in regexp");
                return;
            }
        ...
        } else{ // An iteration token was read
            if ( tokenCount == -1){ doSyntaxError("Initial iteration token");
                return;
            }

            if ( token == '+'){ // Transform '+' in ' *'
                if ( tokenCount == MAX_REGEXP_ITEMS - 1){ doSyntaxError("Too many tokens in regexp");
                    return;
                }
            ...
            if ( (token == '*') || (token == '+')){ iteratorCount++;
            }
        }
    }

    if ( tokenCount == -1){ // No tokens read ?!
    ...
    if ( iteratorCount == 0){ // No iterators, we need just one chain length

        endLength = beginLength;
    }

    for ( int length = beginLength; length <= endLength; length++){ FlagChainList tmpList;
        ...
        while ( !done){ iteratorsFound = 0;
            remember = 0;
            pos = 0; // Number of flags filled in

            for ( int i = 0; i < tokenCount; i++){ if ( typeArray[i] == ' '){ if ( c.chain != NULL){ c.chain[pos] = flagArray[i];
                    }
                    pos++;
                } else{ // typeArray[i] == '*' !  Choose a number

                    iteratorsFound++;
                    if ( iteratorsFound == iteratorCount){ // Last iterator: fill in
                        num = maxIteration - iterationsDone;
                        if ( iteratorCount == 1 || remember){ done = true;  // Only one possible combination or all tried
                        }
                    } else{ num = countArray[i];
                        if ( iteratorsFound == 1){ // First Iterator
                            countArray[i]++;
                            nextIterationsDone++;
                            if ( nextIterationsDone > maxIteration){ // Too many
                                nextIterationsDone -= countArray[i];
                                countArray[i] = 0;
                                remember = true;
                            } else{ remember = false;
                            }
                        } else{ // In between iterator
                            if ( remember){ // Overflow for next iteration
                                countArray[i]++;
                                nextIterationsDone++;
                                if ( nextIterationsDone > maxIteration){ // Too many
                                    nextIterationsDone -= countArray[i];
                    ...
                    // Set num flags
                    for ( int j = 0; j < num; j++){ c.chain[pos] = flagArray[i];
                        pos++;
                    }
                }
            }
