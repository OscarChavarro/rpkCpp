...
class PhotonMapSampler: public BsdfSampler{ private:
    PhotonMap *m_photonMap; // To be used for importance sampling

    static RefractionIndex
    bsdfGeometricIOR(const PhongBidirScattDistFunc *bsdf);

    static bool
    chooseFresnelDirection( SimpleRaytracingPathNode *thisNode, char flags, double x2, Vector3D *dir, double *pdfDir, ColorRgb *scatteringColor, bool *doCosInverse);

    bool
    fresnelSample( VoxelGrid *sceneVoxelGrid, Background *sceneBackground, const SimpleRaytracingPathNode *prevNode, SimpleRaytracingPathNode *thisNode, SimpleRaytracingPathNode *newNode, double x2, char flags);

    bool
    gdSample( Camera *camera, VoxelGrid *sceneVoxelGrid, Background *sceneBackground, SimpleRaytracingPathNode *prevNode, SimpleRaytracingPathNode *thisNode, SimpleRaytracingPathNode *newNode, double x1, double x2, char flags);
    ...
    static bool
    chooseComponent( char flags1, char flags2, const PhongBidirScattDistFunc *bsdf, RayHit *hit, bool doRR, double *x, float *probabilityDensityFunction, bool *chose1);
