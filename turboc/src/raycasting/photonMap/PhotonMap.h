...
class PhotonMap {
  ...
  public:
    static bool
    zeroAlbedo(const PhongBidirScattDistFunc *bsdf, RayHit *hit, char flags);
    ...
    virtual ColorRgb reconstruct(RayHit *hit, Vector3D &outDir,
                                 PhongBidirScattDistFunc *bsdf, PhongBidirScattDistFunc *inBsdf, PhongBidirScattDistFunc *outBsdf);
    ...
    void printStats(PrintStream *stream) const;
