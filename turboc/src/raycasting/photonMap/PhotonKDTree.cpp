...
PhotonKDTree::NormalBQuery_rec(const int index) {
    ...
    dist = PhotonKDTree::sqrDistance3D(((float *)(node.m_data)), qdat_s.point);

    // Normal constraint
    if ( dist < qdat_s.maximumDistance &&
         (((IrrPhoton *)(node.m_data))->Normal().dotProduct(qdat_s.normal) > qdat_s.threshold ) ) {
        // Replace point if distance < maxdist AND normal is similar
        qdat_s.maximumDistance = dist;
        qdat_s.photon = ((IrrPhoton *)(node.m_data));
    }
}
