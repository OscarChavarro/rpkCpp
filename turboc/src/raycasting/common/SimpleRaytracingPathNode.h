...
class SimpleRaytracingPathNode {
  public:
    ...
    */
    ...
    PhongBidirScattDistFunc *m_useBsdf; // bsdf used for scattering
    PhongBidirScattDistFunc *m_inBsdf; // Medium of incoming ray
    PhongBidirScattDistFunc *m_outBsdf; //Medium of a possible transmitted ray (other side of normal)
  ...
  public:
    ...
    PhongBidirScattDistFunc *getPreviousBsdf() const; // Searches backwards for matching node
    void assignBsdfAndNormal(); // Assigns outgoing bsdf for a filled node

    void print(PrintStream *out) const;
