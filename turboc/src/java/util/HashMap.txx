#include "java/util/HashMap.h"
#include <limits.h>


template <class K, class V>
inline size_t
HashMap<K, V>::hashFNV1a(const unsigned char *bytes, size_t length) {
    // Use constants that match the width of size_t to avoid overflow/truncation on 32-bit builds.
#if ULONG_MAX > 0xFFFFFFFFuL
    const size_t offset = ((size_t)(1469598103934665603UL));
    const size_t prime = ((size_t)(1099511628211UL));
#else
    const size_t offset = ((size_t)(2166136261U));
    const size_t prime = ((size_t)(16777619U));
#endif
    size_t hash = offset;
    for ( size_t i = 0; i < length; i++ ) {
        hash ^= ((size_t)(bytes[i]));
        hash *= prime;
    }
    return hash;
}

template <class K, class V>
inline size_t
HashMap<K, V>::hashKeyValue(const K &value) {
    return HashMap<K, V>::hashFNV1a(((const unsigned char *)(&value)), sizeof(K));
}

template <class K, class V>
HashMap<K, V>::HashMap():
    buckets(NULL),
    bucketCount(0),
    elementCount(0),
    maxLoadFactor(0.75f)
{
    initialize(16);
}

template <class K, class V>
HashMap<K, V>::HashMap(long initialCapacity):
    buckets(NULL),
    bucketCount(0),
    elementCount(0),
    maxLoadFactor(0.75f)
{
    initialize(initialCapacity <= 0 ? 16 : initialCapacity);
}

template <class K, class V>
HashMap<K, V>::~HashMap() {
    clear();
}

template <class K, class V>
void
HashMap<K, V>::initialize(long initialBucketCount) {
    if ( initialBucketCount <= 0 ) {
        initialBucketCount = 16;
    }

    buckets = new Entry *[initialBucketCount];
    if ( buckets == NULL ) {
        bucketCount = 0;
        return;
    }

    bucketCount = initialBucketCount;
    for ( long i = 0; i < bucketCount; i++ ) {
        buckets[i] = NULL;
    }
}

template <class K, class V>
void
HashMap<K, V>::clear() {
    if ( buckets != NULL ) {
        for ( long i = 0; i < bucketCount; i++ ) {
            Entry *current = buckets[i];
            while ( current != NULL ) {
                Entry *next = current->next;
                delete current;
                current = next;
            }
            buckets[i] = NULL;
        }
        delete[] buckets;
        buckets = NULL;
    }

    bucketCount = 0;
    elementCount = 0;
}

template <class K, class V>
long
HashMap<K, V>::size() const {
    return elementCount;
}

template <class K, class V>
bool
HashMap<K, V>::isEmpty() const {
    return elementCount == 0;
}

template <class K, class V>
long
HashMap<K, V>::bucketIndexFor(const K &key) const {
    if ( bucketCount <= 0 ) {
        return 0;
    }
    const size_t hashValue = hashKeyValue(key);
    return ((long)(hashValue % ((size_t)(bucketCount))));
}

template <class K, class V>
bool
HashMap<K, V>::containsKey(const K &key) const {
    return tryGet(key, NULL);
}

template <class K, class V>
bool
HashMap<K, V>::tryGet(const K &key, V *valueOut) const {
    if ( buckets == NULL || bucketCount <= 0 ) {
        return false;
    }

    const long index = bucketIndexFor(key);
    Entry *current = buckets[index];
    while ( current != NULL ) {
        if ( current->key == key ) {
            if ( valueOut != NULL ) {
                *valueOut = current->value;
            }
            return true;
        }
        current = current->next;
    }

    return false;
}

template <class K, class V>
V
HashMap<K, V>::getOrDefault(const K &key, const V &defaultValue) const {
    V value = defaultValue;
    if ( tryGet(key, &value) ) {
        return value;
    }
    return defaultValue;
}

template <class K, class V>
void
HashMap<K, V>::rehash(long newBucketCount) {
    if ( newBucketCount <= bucketCount || buckets == NULL ) {
        return;
    }

    Entry **newBuckets = new Entry *[newBucketCount];
    if ( newBuckets == NULL ) {
        return;
    }
    for ( long i = 0; i < newBucketCount; i++ ) {
        newBuckets[i] = NULL;
    }

    for ( long i = 0; i < bucketCount; i++ ) {
        Entry *current = buckets[i];
        while ( current != NULL ) {
            Entry *next = current->next;
            const size_t hashValue = hashKeyValue(current->key);
            const long newIndex = ((long)(hashValue % ((size_t)(newBucketCount))));
            current->next = newBuckets[newIndex];
            newBuckets[newIndex] = current;
            current = next;
        }
    }

    delete[] buckets;
    buckets = newBuckets;
    bucketCount = newBucketCount;
}

template <class K, class V>
bool
HashMap<K, V>::put(const K &key, const V &value) {
    if ( buckets == NULL || bucketCount <= 0 ) {
        initialize(16);
        if ( buckets == NULL ) {
            return false;
        }
    }

    const long index = bucketIndexFor(key);
    Entry *current = buckets[index];
    while ( current != NULL ) {
        if ( current->key == key ) {
            current->value = value;
            return true;
        }
        current = current->next;
    }

    Entry *entry = new Entry(key, value, buckets[index]);
    if ( entry == NULL ) {
        return false;
    }
    buckets[index] = entry;
    elementCount++;

    const float maxElements = ((float)(bucketCount)) * maxLoadFactor;
    if ( ((float)(elementCount)) > maxElements ) {
        rehash(bucketCount * 2);
    }
    return true;
}

template <class K, class V>
bool
HashMap<K, V>::remove(const K &key) {
    if ( buckets == NULL || bucketCount <= 0 ) {
        return false;
    }

    const long index = bucketIndexFor(key);
    Entry *current = buckets[index];
    Entry *previous = NULL;
    while ( current != NULL ) {
        if ( current->key == key ) {
            if ( previous == NULL ) {
                buckets[index] = current->next;
            } else {
                previous->next = current->next;
            }
            delete current;
            elementCount--;
            return true;
        }
        previous = current;
        current = current->next;
    }

    return false;
}
