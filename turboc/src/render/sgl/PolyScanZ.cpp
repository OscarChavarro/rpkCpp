...
Poly::scanlineZ(const SglContext *sglContext, int y, const PolygonVertex *l, const PolygonVertex *r, const Window *win) {
    ...
    for ( int x = lx; x <= rx; x++ ) {
        ...
        if ( z <= sglContext->depthBuffer[pixelIndex] ) {
            if ( sglContext->pixelData == PATCH_POINTER ) {
                sglContext->patchBuffer[pixelIndex] = ((Patch *)(sglContext->currentPatch));
            } else if ( sglContext->pixelData == ELEMENT_POINTER ) {
                sglContext->galerkinElementBuffer[pixelIndex] =
                    ((Element *)(sglContext->currentGalerkinElement));
            } else {
...
Poly::scanZ(SglContext *sglContext, Polygon *p,  const Window *window)
{
    ...
    PolygonVertex l = PolygonVertex();
    PolygonVertex r = PolygonVertex();
    PolygonVertex dl = PolygonVertex();
    PolygonVertex dr = PolygonVertex();
