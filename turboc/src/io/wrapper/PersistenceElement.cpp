...
int
PersistenceElement::readByteInt(InputStream &is) {
    readBytes(is, byteBuffer1byte, 1);
    return ((int)(((signed char)(byteBuffer1byte[0]))));
}

int
PersistenceElement::readByteUnsignedInt(InputStream &is) {
    readBytes(is, byteBuffer1byte, 1);
    return signedByte2unsignedInteger(byteBuffer1byte[0]);
}

void
PersistenceElement::writeByte(OutputStream &os, unsigned char value) {
    writeBytes(os, &value, 1);
}

void
PersistenceElement::writeBool(OutputStream &os, bool value) {
    writeByte(os, ((unsigned char)(value ? 1 : 0)));
}
    ...
    const int inNumberToConvert)
{
    ...
    for ( i = 0; i < length; i++ ) {
        byteBuffer2byte[i] = ((unsigned char)((inNumberToConvert & (0xFF << 8 * i)) >> (8 * i)));
    }
    ...
    int inNumberToConvert)
{
    ...
    for ( i = 0; i < length; i++ ) {
        byteBuffer2byte[length - i - 1] = ((unsigned char)((inNumberToConvert & (0xFF << 8 * i)) >> (8 * i)));
    }
...
PersistenceElement::signedInt2byteArrayDirect(unsigned char *arr, int start, long num) {
    ...
    for ( i = 0; i < len; i++ ) {
        tmp[i] = ((unsigned char)((num & (0xFFL << (8 * i))) >> (8 * i)));
    }
...
PersistenceElement::signedInt2byteArrayInvert(unsigned char *arr, int start, long num) {
    ...
    for ( i = 0; i < len; i++ ) {
        tmp[len - i - 1] = ((unsigned char)((num & (0xFFL << (8 * i))) >> (8 * i)));
    }
...
PersistenceElement::byteArray2floatDirect(const unsigned char *arr, int start) {
    ...
    i = 0;
    for ( int shiftBy = 0; shiftBy < 32; shiftBy += 8 ) {
        accum |= ((unsigned int)(tmp[i] & 0xff)) << shiftBy;
        i++;
    }

    float out = 0.0f;
    memcpy(&out, &accum, sizeof(float));
    return out;
}
...
PersistenceElement::byteArray2floatBE(const unsigned char *arr, int start) {
    if ( bigEndianArchitecture ) {
        return ((float)(byteArray2longDirect(arr, start)));
    }
    return ((float)(byteArray2longInvert(arr, start)));
}
...
int
PersistenceElement::readSignedShortLE(InputStream &is) {
    readBytes(is, byteBuffer2byte, 2);
    return byteArray2signedShortLE(byteBuffer2byte, 0);
}

int
PersistenceElement::readSignedShortBE(InputStream &is) {
...
void
PersistenceElement::writeSignedShortBE(OutputStream &os, int num) {
    signedShort2byteArrayBE(byteBuffer2byte, 0, num);
    writeBytes(os, byteBuffer2byte, 2);
}

void
PersistenceElement::writeSignedShortLE(OutputStream &os, int num) {
...
*/
long
PersistenceElement::readLongLE(InputStream &is) {
    readBytes(is, bytesForLong, 4);
    return byteArray2longLE(bytesForLong, 0);
}

void
PersistenceElement::writeInt32LE(OutputStream &os, int num) {
    writeLongLE(os, ((long)(num)));
}
...
*/
long
PersistenceElement::readLongBE(InputStream &is) {
    readBytes(is, bytesForLong, 4);
    return byteArray2longBE(bytesForLong, 0);
}

float
PersistenceElement::readFloatLE(InputStream &is) {
    readBytes(is, byteBuffer4byte, 4);
    return byteArray2floatLE(byteBuffer4byte, 0);
}

float
PersistenceElement::readFloatBE(InputStream &is) {
    readBytes(is, byteBuffer4byte, 4);
    const long i = byteArray2longBE(byteBuffer4byte, 0);
    const unsigned int j = ((unsigned int)(i));
    float out = 0.0f;
    memcpy(&out, &j, sizeof(float));
    return out;
}

void
PersistenceElement::writeFloatBE(OutputStream &os, float num) {
    float2byteArrayBE(byteBuffer4byte, 0, num);
    writeBytes(os, byteBuffer4byte, 4);
}

void
PersistenceElement::writeFloatLE(OutputStream &os, float num) {
    float2byteArrayLE(byteBuffer4byte, 0, num);
    writeBytes(os, byteBuffer4byte, 4);
}

void
PersistenceElement::writeLongBE(OutputStream &os, long num) {
    if ( bigEndianArchitecture ) {
...
void
PersistenceElement::writeLongLE(OutputStream &os, long num) {
    if ( bigEndianArchitecture ) {
        signedInt2byteArrayInvert(bytesForLong, 0, num);
    }
    signedInt2byteArrayDirect(bytesForLong, 0, num);
...
char *
PersistenceElement::readAsciiFixedSizeString(InputStream &is, int size) {
    if ( size <= 0 ) {
        return duplicateCString("");
    }

    ArrayList<unsigned char> bytesForString(size);
...
char *
PersistenceElement::readAsciiString(InputStream &is) {
    unsigned char character[1] = {0};
    ArrayList<unsigned char> bytes(64);
...
char *
PersistenceElement::readUtf8String(InputStream &is) {
    unsigned char character[1] = {0};
    ...
    do {
        ...
        if ( character[0] != 0x00 && ((character[0] >> 7) == 0) ) {
            ...
            if ( buildUtf8Char(pair, utf8Char) ) {
                ...
                System::err.print("* UNHANDLED UTF sequence while reading UTF-8 string\n");
            }
        }
    } while ( character[0] != 0x00 );
...
char *
PersistenceElement::readUtf8Line(InputStream &is) {
    unsigned char character[1] = {0};
    unsigned char pair[2] = {0, 0};
    unsigned char utf8Char[2] = {0, 0};
    ArrayList<unsigned char> bytes(64);
...
char *
PersistenceElement::readAsciiLine(InputStream &is) {
    unsigned char character[1] = {0};
    ArrayList<unsigned char> bytes(64);
...
char *
PersistenceElement::readAsciiToken(InputStream &is, const unsigned char *separators, int separatorsLength) {
    unsigned char character[1] = {0};
    ArrayList<unsigned char> bytes(64);
...
void
PersistenceElement::writeAsciiString(OutputStream &writer, const char *cad) {
    const char *text = cad == NULL ? "" : cad;
    const int textLength = ((int)(strlen(text)));
    if ( textLength > 0 ) {
        writeBytes(writer, ((const unsigned char *)(text)), textLength);
    }
...
void
PersistenceElement::writeUtf8String(OutputStream &writer, const char *cad) {
    const char *text = cad == NULL ? "" : cad;
    const int textLength = ((int)(strlen(text)));
    if ( textLength > 0 ) {
        writeBytes(writer, ((const unsigned char *)(text)), textLength);
...
void
PersistenceElement::writeAsciiLine(OutputStream &writer, const char *cad) {
    const char *text = cad == NULL ? "" : cad;
    const int textLength = ((int)(strlen(text)));
    if ( textLength > 0 ) {
        writeBytes(writer, ((const unsigned char *)(text)), textLength);
...
void
PersistenceElement::writeUtf8Line(OutputStream &writer, const char *cad) {
    const char *text = cad == NULL ? "" : cad;
    const int textLength = ((int)(strlen(text)));
    if ( textLength > 0 ) {
        writeBytes(writer, ((const unsigned char *)(text)), textLength);
...
PersistenceElement::joinCString2(const char *left, const char *right) {
    const char *leftText = left == NULL ? "" : left;
    const char *rightText = right == NULL ? "" : right;

    const size_t leftLength = strlen(leftText);
    const size_t rightLength = strlen(rightText);
    char *joined = ((char *)(malloc(leftLength + rightLength + 1)));
    if ( joined == NULL ) {
        return NULL;
    }

    memcpy(joined, leftText, leftLength);
    memcpy(joined + leftLength, rightText, rightLength);
...
PersistenceElement::joinCString3(const char *first, const char *second, const char *third) {
    const char *firstText = first == NULL ? "" : first;
    const char *secondText = second == NULL ? "" : second;
    const char *thirdText = third == NULL ? "" : third;

    const size_t firstLength = strlen(firstText);
    const size_t secondLength = strlen(secondText);
    const size_t thirdLength = strlen(thirdText);
    char *joined = ((char *)(malloc(firstLength + secondLength + thirdLength + 1)));
    if ( joined == NULL ) {
        return NULL;
    }

    memcpy(joined, firstText, firstLength);
    memcpy(joined + firstLength, secondText, secondLength);
    memcpy(joined + firstLength + secondLength, thirdText, thirdLength);
...
PersistenceElement::containsExistingLibrary(const char *pathList, char pathSeparator, const char *nativeLibname) {
    if ( pathList == NULL || nativeLibname == NULL || nativeLibname[0] == '\0' ) {
        return false;
    }
    ...
    while ( tokenStart != NULL ) {
        ...
        const size_t tokenLength = ((size_t)(cursor - tokenStart));
        if ( tokenLength > 0 ) {
...
*/
char *
PersistenceElement::extractExtensionFromFile(const File &fd) {
