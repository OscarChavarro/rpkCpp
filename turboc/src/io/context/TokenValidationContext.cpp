...
{
    int index = 0;
    while ( Character::isSpace(text[index]) ) {
        index++;
    }
...
{
    int startIndex = 0;
    while ( Character::isSpace(text[startIndex]) ) {
        startIndex++;
    }
    ...
    while ( Character::isDigit(text[currentIndex]) ) {
        currentIndex++;
    }
...
{
    const char *cp = skipInt(text);
    return cp != NULL && strchr(delimiters, *cp) != NULL;
}
...
TokenValidationContext::isName(const char *text)
{
    ...
    if ( !TokenValidationContext::isAsciiCode(text[index]) || !Character::isLetter(text[index]) ) {
        // start with a letter
        return false;
    }
