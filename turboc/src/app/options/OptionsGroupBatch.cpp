...
{
    TypedOption<int> iterationsOpt("-iterations", &batchOptionsState.iterations, 1, NULL, NULL);
    TypedOption<const char *> obfOpt("-obf", &batchOptionsState.binaryOutputFilename, 1, OptionsGroupBatch::binaryOutputOption, NULL);
    TypedOption<const char *> ibfOpt("-ibf", &batchOptionsState.binaryInputFilename, 1, OptionsGroupBatch::binaryInputOption, NULL);
    TypedOption<const char *> radianceImageOpt("-radiance-image-savefile", &batchOptionsState.radianceImageFileNameFormat, 1, NULL, NULL);
    TypedOption<const char *> radianceModelOpt("-radiance-model-savefile", &batchOptionsState.radianceModelFileNameFormat, 1, NULL, NULL);
    TypedOption<int> saveModuloOpt("-save-modulo", &batchOptionsState.saveModulo, 1, NULL, NULL);
    TypedOption<const char *> raytracingImageOpt("-raytracing-image-savefile", &batchOptionsState.raytracingImageFileName, 1, NULL, NULL);
    TypedOption<int> timingsOpt("-timings", &batchOptionsState.timings, 0, OptionsGroupBatch::setIntTrue, NULL);
