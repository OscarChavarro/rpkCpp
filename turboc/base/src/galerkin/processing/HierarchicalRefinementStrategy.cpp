...
*/
double
HierarchicalRefinementStrategy::hierRefLinkErrrThrsh(
    ...
    switch ( galerkinState->galerkinIterationMethod ) {
        ...
        case SOUTH_WELL:
            ...
            if ( galerkinState->importanceDriven && interaction->receiverElement->isCluster() ) {
                ...
                switch ( galerkinState->errorNorm ) {
                    case RADIANCE_ERROR:
                        approxError2 *= hierRefClrTErrr(
                            Statistics::instance().radiance.maxSelfEmittedRadiance) / Statistics::instance().potential.maxDirectPotential;
                        break;
                    case POWER_ERROR:
                        approxError2 *= hierRefClrTErrr(
                            Statistics::instance().radiance.maxSelfEmittedPower) / M_PI / Statistics::instance().potential.maxDirectImportance;
                        break;
                }
    ...
    GalerkinState *galerkinState)
{
    ...
    error.abs();
    return hierRefClrTErrr(error);
}

InteractionEvaluationCode
HierarchicalRefinementStrategy::hierRefEvalInter(
    Interaction *interaction,
    GalerkinState *galerkinState)
{
    ColorRgb srcRho;
    ...
    threshold = hierRefLinkErrrThrsh(interaction, receiveArea, galerkinState);
    error = hierRefApprxErrr(interaction, srcRho, rcvRho, galerkinState);

    if ( interaction->sourceElement->isCluster()
         && error < threshold
         && galerkinState->clusteringStrategy != ISOTROPIC ) {
        error += srcClustRadnVrtnErrr(interaction, rcvRho, receiveArea, galerkinState);
    }
    ...
    if ( error > threshold ) {
        ...
        if ( (!(interaction->sourceElement->isCluster()
            && (interaction->sourceElement->flags & IS_LIGHT_SOURCE_MASK)) )
...
*/
void
HierarchicalRefinementStrategy::hierRefCompLightTransp(
    Interaction *interaction,
    GalerkinState *galerkinState)
{
    // Update the number of effectively used radiance coefficients on the
    // receiver element
    int a = Math::min(interaction->nmbrOBasisFnctnORecv, interaction->receiverElement->basisSize);
...
*/
int
HierarchicalRefinementStrategy::hierRefCreateSubdivLink(
    const Scene *scene,
    ...
    const GalerkinState *galerkinState)
{
    ...
    if ( interaction->receiverElement->isCluster() ) {
        interaction->nmbrOBasisFnctnORecv = 1;
    } else {
        interaction->nmbrOBasisFnctnORecv = receiverElement->basisSize;
    }
...
*/
void
HierarchicalRefinementStrategy::hierRefStoreInter(
    Interaction *interaction,
    const GalerkinState *galerkinState)
{
...
*/
void
HierarchicalRefinementStrategy::hierRefRegSbdvdSrc(
    const Scene *scene,
    ArrayList<Geometry *> **candidatesList,
    Interaction *interaction,
    bool isClusteredGeometry,
    GalerkinState *galerkinState)
{
    ArrayList<Geometry *> *backup = *candidatesList;
...
*/
void
HierarchicalRefinementStrategy::hierRefRegSbdvdRecv(
    const Scene *scene,
    ArrayList<Geometry *> **candidatesList,
    Interaction *interaction,
    bool isClusteredGeometry,
...
*/
void
HierarchicalRefinementStrategy::hierRefSbdvdSrcClust(
    const Scene *scene,
    ArrayList<Geometry *> **candidatesList,
    Interaction *interaction,
    bool isClusteredGeometry,
...
*/
void
HierarchicalRefinementStrategy::hierRefSbdvdRecvClust(
    const Scene *scene,
    ArrayList<Geometry *> **candidatesList,
    Interaction *interaction,
    bool isClusteredGeometry,
    ...
    GalerkinState *galerkinState)
{
    ...
    switch ( hierRefEvalInter(interaction, galerkinState) ) {
        case ACCURATE_ENOUGH:
            hierRefCompLightTransp(interaction, galerkinState);
            refined = false;
            break;
