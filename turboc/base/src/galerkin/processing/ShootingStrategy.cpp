...
*/
Patch *
ShootingStrategy::chooseRadianceShootingPatch(const ArrayList<Patch *> *scenePatches, const GalerkinState *galerkinState) {
    ...
    shooting_patch = pot_shooting_patch = NULL;
...
ShootingStrategy::clearUnShotRadianceAndPotential(GalerkinElement *elem) {
    ...
    ColorRgbMutable::arrayClear(elem->unShotRadiance, elem->basisSize);
...
*/
void
ShootingStrategy::ptchPropUnShotRadnAPot(
    const Scene *scene,
    const Patch *patch,
...
ShootingStrategy::shootingPushPullPotential(GalerkinElement *element, float down) {
    ...
    if ( element->irregularSubElements ) {
        for ( int j = 0; element->irregularSubElements != NULL && j < element->irregularSubElements->size(); j++ ) {
...
ShootingStrategy::doPropagate(const Scene *scene, const Patch *shootingPatch, GalerkinState *galerkinState) {
    ...
    ptchPropUnShotRadnAPot(scene, shootingPatch, galerkinState);
...
*/
Patch *
ShootingStrategy::choosePotentialShootingPatch(const ArrayList<Patch *> *scenePatches) {
    float maximumImportance = 0.0f;
    Patch *shootingPatch = NULL;

    for ( int i = 0; scenePatches != NULL && i < scenePatches->size(); i++ ) {
        Patch *patch = scenePatches->get(i);
        float imp = patch->area * Math::abs(galerkinGetUnShotPotential(patch));
