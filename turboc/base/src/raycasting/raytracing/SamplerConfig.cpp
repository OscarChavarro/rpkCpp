...
void
SamplerConfig::init(bool useQMC, int qmcDepth) {
    m_useQMC = useQMC;
    m_qmcDepth = qmcDepth;
...
void
SamplerConfig::getRand(int depth, double *x1, double *x2) const {
        ...
        *x2 = drand48();
        ...
        if ( depth == 0 || depth == 2 ) {
            ...
            *x2 = nrs[1] * Niederreiter31::RECIP;
            ...
            System::out.printf("Hmmmm MD %i D%i\n", m_qmcDepth, depth);
            *x1 = drand48();
            *x2 = drand48();
        }
    }
    ...
    char flags)
{
    ...
    bool continueTrace = nextNode != NULL;
    ...
    Vector3D *pDirEl)
{
    ...
    if ( nodeLP == NULL ) {
        ...
        if ( nodeY->m_hit.getMaterial()->getEdf() == NULL ) {
            ...
            ShadingContext context = nodeY->m_hit.shadingContext(NULL);
            nodeY->m_bsdfEval = nodeY->m_hit.getMaterial()->getEdf()->phongEdfEval(
                &context, &dirLE, bsdfFlagsL, NULL);
        }
