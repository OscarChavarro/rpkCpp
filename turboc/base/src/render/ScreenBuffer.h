...
class ScreenBuffer {
  ...
  public:
    explicit ScreenBuffer(const Camera *camera, const Camera *defaultCamera, ToneMappingContext *toneMapOptions = NULL); // Also calls mainInitApplication()
    ~ScreenBuffer();
    ...
    void writeFile(const char *fileName, OutputStream *outputStream, int isPipe);
