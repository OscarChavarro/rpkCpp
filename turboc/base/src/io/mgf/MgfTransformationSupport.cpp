...
*/
long
MgfTransformationSupport::computeUniqueId(const Matrix4x4d *xfm) {
    ...
    for ( long unsigned int i = 0; i < sizeof(Matrix4x4d) / sizeof(unsigned short); i++ ) {
        xid ^= ((long)((((const unsigned short *)(xfm->m)))[i] << shiftTab[i & 63]));
    }
    return xid;
}

double
MgfTransformationSupport::d2r(double a) {
    return (M_PI / 180.0) * a;
}
...
*/
int
MgfTransformationSupport::checkForBadArguments(int ac, char **av, const char *fl) {
    if ( fl == NULL ) {
        // No arguments?
        fl = "";
    }
    for ( int formatIndex = 0; fl[formatIndex] != '\0'; formatIndex++ ) {
        ...
        switch ( fl[formatIndex] ) {
            case 's': // String
                if ( av[formatIndex][0] == '\0' || Character::isSpace(av[formatIndex][0]) ) {
...
bool
MgfTransformationSupport::checkArgument(int a, const char *l, int ac, char **av, int i) {
...
*/
int
MgfTransformationSupport::transformName(const TransformSequenceContext *ap, ParseRuntimeContext *context) {
    static char oName[10 * TransformSequenceContext::TRANSFORM_MAXIMUM_DIMENSIONS];
    static const char *oav[3] = {
        context->entityNames[OBJECT], oName
    };
    if ( ap == NULL ) {
        return MgfEntityControl::mgfHandle(OBJECT, 1, oav, context);
    }
    int writeIndex = 0;
    oName[writeIndex++] = 'a';
    ...
    oName[writeIndex] = '\0';
    return MgfEntityControl::mgfHandle(OBJECT, 2, oav, context);
}
...
MgfTransformationSupport::newTransform(int ac, const char **av, ParseRuntimeContext *context) {
    ...
    if ( ac > 0 ) {
        ...
        if ( spec->ownedArgumentCopies == NULL ) {
            delete spec;
            return NULL;
        }
        for ( int i = 0; i < ac; i++ ) {
    ...
    if ( nDim != 0 ) {
        ...
        MgfEntityControl::mgfGetFilePosition(&spec->transformationArray->startingPosition, context);
    ...
    char **specArguments = ac > 0 ? stack.argumentVectorFor(spec) : NULL;
    for ( int i = 0; i < ac; i++ ) {
        if ( !strcmp(av[i], "-a") ) {
            ...
            spec->transformationArray->transformArguments[spec->transformationArray->numberOfDimensions++].n = ((short)(strtol(av[i], NULL, 10)));
        } else {
...
*/
void
MgfTransformationSupport::mgfTransformPoint(Vector3Dd *v1, const Vector3Dd *v2, const ParseRuntimeContext *context) {
...
*/
void
MgfTransformationSupport::mgfTransformVector(Vector3Dd *v1, const Vector3Dd *v2, const ParseRuntimeContext *context) {
    if ( context->transformContext == NULL) {
        v1->copy(v2);
        return;
    }
...
void
MgfTransformationSupport::finish(int count, TransformContext *ret, const Matrix4x4d *transformMatrix, double scaTransform) {
    while ( count-- > 0 ) {
...
*/
int
MgfTransformationSupport::xf(TransformContext *ret, int ac, char **av) {
    ...
    for ( i = 0; i < ac && av[i][0] == '-'; i++ ) {
        ...
        switch ( av[i][1] ) {
            ...
            case 't':
                ...
                m4.m[3][0] = strtod(av[++i], NULL);
                m4.m[3][1] = strtod(av[++i], NULL);
                m4.m[3][2] = strtod(av[++i], NULL);
                break;
            ...
            case 'r':
                ...
                switch ( av[i][2] ) {
                    ...
                    default: {
                        ...
                        float x = strtof(av[++i], NULL);
                        float y = strtof(av[++i], NULL);
                        float z = strtof(av[++i], NULL);
                        float a = ((float)(d2r(strtod(av[++i], NULL))));
                        float s = Math::sqrt(x * x + y * y + z * z);
                        x /= s;
                        y /= s;
                        z /= s;
                        float c = Math::cos(a);
            ...
            case 'i':
                ...
                counter = ((int)(strtol(av[++i], NULL, 10)));
                transformMatrix.identity();
                scaTransform = 1.0;
                continue;
...
bool
MgfTransformationSupport::compactTransformArguments(ParseRuntimeContext *context, const TransformStackContext *stackContext) {
...
*/
int
MgfTransformationSupport::handleTransformationEntity(int ac, const char **av, ParseRuntimeContext *context) {
    TransformScopeContext &stack = context->transformStack;
    ...
    if ( ac == 1 ) {
        ...
        if ( spec->transformationArray != NULL) {
            ...
            if ( n >= 0 ) {
                ...
                Formatter::format(ap->transformArguments[n].arg, 8, "%d", ap->transformArguments[n].i);
    ...
    if ( spec->prev != NULL) {
        ...
        spec->rev = ((short)(spec->rev ^ spec->prev->rev));
    }
...
void
MgfTransformationSupport::mgfTransformFreeMemory(ParseRuntimeContext *context) {
    if ( context != NULL ) {
