...
def convert_to_dos_text(project_root: Path, dry_run: bool):
    cmd = shutil.which("todos")
    if cmd is None:
        print('[warn] "todos" not found in PATH; skipping DOS text conversion.')
        return

    targets = [p for p in sorted(project_root.rglob("*")) if p.is_file() and p.suffix.lower() in {".cpp", ".h"}]
    if not targets:
        print("No .cpp/.h files found for DOS text conversion.")
        return

    print(f'Phase 4: converting {len(targets)} files to DOS text with "todos"')
    if dry_run:
        for path in targets:
            print(f"  [dry-run] todos {path.relative_to(project_root)}")
        return
...
def build_target_upper_dirs(mappings):
    targets = set()
    for _old_rel, new_rel in mappings:
        parts = Path(new_rel.replace("\\", "/")).parts[:-1]
        for depth in range(1, len(parts) + 1):
            targets.add("/".join(parts[:depth]).upper())
    return targets


def uppercase_directories(project_root: Path, mappings, dry_run: bool):
    target_dirs = build_target_upper_dirs(mappings)
    ...
    for src in dirs:
        ...
        if rel.upper() not in target_dirs:
            continue
        name_up = src.name.upper()
        if name_up == src.name:
            continue
        dst = src.with_name(name_up)
        candidates.append((src, dst))

    if not candidates:
        print("Phase 5: no directories needed uppercase rename.")
        return

    print(f"Phase 5: renaming {len(candidates)} directories to uppercase")
    for src, dst in candidates:
        rename_with_temp(src, dst, dry_run, "dirupper")


def apply_to_83(project_root: Path, mappings, dry_run: bool, strict: bool):
    steps = []
    for old_rel, new_rel in mappings:
    ...
    print(f"Phase 2: moving temp names to final 8.3 names")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Apply mappings.csv from normal names to 8.3 names")
    ...
    apply_to_83(project_root, mappings, args.dry_run, args.strict)
    print("Phase 3: updating quoted #include directives")
    update_quoted_includes(project_root, mappings, args.dry_run)
    convert_to_dos_text(project_root, args.dry_run)
    uppercase_directories(project_root, mappings, args.dry_run)
    return 0
